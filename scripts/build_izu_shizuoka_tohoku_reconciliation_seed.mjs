...
const candidateRoot = path.join(repoRoot, 'research/onsen-candidates/nationwide-2026-07-03');
const izuCandidatePath = path.join(candidateRoot, 'izu_shizuoka_accommodation_candidate_status_2026-07-03.csv');
const nationwideMasterPath = path.join(candidateRoot, 'nationwide_accommodation_master_v0_6_2026-07-03.csv');
const outputDir = path.join(repoRoot, 'research/onsen-db-seed');
const outputBase = 'izu_shizuoka_tohoku_reconciliation_seed_2026-07-08';
...
const tohokuPrefectures = new Set(['青森県', '岩手県', '宮城県', '秋田県', '山形県', '福島県']);
const allowedStatuses = /ready/;

const displayNameKo = {
  'atami-fufu': '후후 아타미',
  'atami-sekaie': '아타미 세카이에',
  'ito-futarikomorebi': '호텔 후타리 코모레비',
  'atami-karaku': '아타미 이즈산 가라쿠',
  'atami-kaihourou': '아타미 카이호로',
  'atami-new-tomiyoshi': '뉴 토미요시',
  'atami-kamenoi': '가메노이 호텔 아타미',
  'atami-pearl-star': '아타미 펄 스타 호텔',
  'atami-korakuen': '아타미 고라쿠엔 호텔',
  'atami-new-akao': '호텔 뉴 아카오',
  'atami-watei-kazekomichi': '와테이 가제코미치',
  'atami-soki': '소키 아타미',
  'atami-sekitei': '아타미 세키테이',
  'ito-kazenokaori-umi': '가제노카오리 우미',
  'ito-gensen-tsuki': '겐센토 하나레노오야도 츠키',
  'ito-juraku': '이토 호텔 주라쿠',
  'ito-yumenoya': '오야도 유메노야',
  'ito-akazawa-geihinkan': '아카자와 게이힌칸',
  'akiu-saryo-soen': '차료 소엔',
  'akiu-sakurareikyu': '사쿠라 리큐',
  'zao-daikon-no-hana': '다이콘노하나',
  'akiu-ryokusuitei': '가가리비노유 료쿠스이테이',
  'akiu-hotel-zuiho': '호텔 즈이호',
  'nyuto-tsurunoyu': '츠루노유 온천',
  'nyuto-taenoyu': '다에노유',
  'nyuto-kuroyu': '구로유 온천',
  'nyuto-ganiba': '가니바 온천',
  'nyuto-kyukamura': '규카무라 뉴토온천향',
  'ginzan-fujiya': '긴잔온천 후지야',
  'ginzan-notoya': '긴잔온천 노토야 료칸',
  'ginzan-kosekiya': '긴잔온천 고세키야 별관',
  'zao-miyagiya': '자오온천 미야기야',
  'zao-lucent': '자오온천 호텔 루센트',
  'kaminoyama-koyo': '니혼노야도 고요',
  'kaminoyama-meigetsuso': '메이게츠소',
  'sukayu-onsen': '스카유 온천 료칸',
  'asamushi-kaisenkaku': '난부야 가이센카쿠',
  'hanamaki-yusen-shidate': '유센 시다테',
  'hanamaki-kashoen': '가쇼엔',
  'hanamaki-osawa-onsen': '오사와온천 산스이카쿠',
  'tsunagi-shikitei': '쓰나기온천 시키테이',
  'higashiyama-mukaitaki': '무카이타키',
  'higashiyama-shosuke': '쇼스케노야도 다키노유',
  'iizaka-yoshikawaya': '요시카와야',
};

const identityOverrides = {
  'iizaka-yoshikawaya': {
    displayNameKo: '스리아게테이 오오토리',
    nameJa: '飯坂温泉 摺上亭大鳥',
    nameSourceNote: '후보 slug는 yoshikawaya로 되어 있으나 딥리서치 산출물의 실제 숙소명은 摺上亭大鳥로 확인되어 판정 seed 이름을 보정했습니다.',
  },
  'zao-miyagiya': {
    displayNameKo: '미야마소 타카미야',
    nameJa: '蔵王温泉 深山荘高見屋',
    nameEn: 'Miyamaso Takamiya',
    nameSourceNote: '후보 slug는 miyagiya로 되어 있으나 딥리서치 산출물의 실제 숙소명은 深山荘高見屋로 확인되어 판정 seed 이름을 보정했습니다.',
  },
  'zao-lucent': {
    displayNameKo: '자오 국제호텔',
    nameJa: '蔵王温泉 蔵王国際ホテル',
    nameEn: 'Zao Kokusai Hotel',
    nameSourceNote: '후보 slug는 lucent로 되어 있으나 딥리서치 산출물의 실제 숙소명은 蔵王国際ホテル로 확인되어 판정 seed 이름을 보정했습니다.',
  },
};

const areaMeta = {
  atami: ['atami', '이즈/시즈오카 · 시즈오카현 · 아타미', 'chubu', 'shizuoka', 'atami', 'atami'],
  'ito-izu': ['ito-izu', '이즈/시즈오카 · 시즈오카현 · 이토/이즈고원', 'chubu', 'shizuoka', 'ito', 'ito-izu'],
  'izu-nagaoka': ['izu-nagaoka', '이즈/시즈오카 · 시즈오카현 · 이즈나가오카', 'chubu', 'shizuoka', 'izunokuni', 'izu-nagaoka'],
  akiu: ['akiu', '도호쿠 · 미야기현 · 아키우', 'tohoku', 'miyagi', 'sendai', 'akiu'],
  'miyagi-zao': ['miyagi-zao', '도호쿠 · 미야기현 · 미야기 자오', 'tohoku', 'miyagi', 'zao', 'miyagi-zao'],
  nyuto: ['nyuto', '도호쿠 · 아키타현 · 뉴토온천향', 'tohoku', 'akita', 'semboku', 'nyuto'],
  ginzan: ['ginzan', '도호쿠 · 야마가타현 · 긴잔온천', 'tohoku', 'yamagata', 'obanazawa', 'ginzan'],
  'zao-yamagata': ['zao-yamagata', '도호쿠 · 야마가타현 · 자오온천', 'tohoku', 'yamagata', 'yamagata', 'zao-yamagata'],
  kaminoyama: ['kaminoyama', '도호쿠 · 야마가타현 · 가미노야마', 'tohoku', 'yamagata', 'kaminoyama', 'kaminoyama'],
  sukayu: ['sukayu', '도호쿠 · 아오모리현 · 스카유', 'tohoku', 'aomori', 'aomori', 'sukayu'],
  asamushi: ['asamushi', '도호쿠 · 아오모리현 · 아사무시', 'tohoku', 'aomori', 'aomori', 'asamushi'],
  hanamaki: ['hanamaki', '도호쿠 · 이와테현 · 하나마키', 'tohoku', 'iwate', 'hanamaki', 'hanamaki'],
  tsunagi: ['tsunagi', '도호쿠 · 이와테현 · 쓰나기', 'tohoku', 'iwate', 'morioka', 'tsunagi'],
  higashiyama: ['higashiyama', '도호쿠 · 후쿠시마현 · 아이즈 히가시야마', 'tohoku', 'fukushima', 'aizuwakamatsu', 'higashiyama'],
  iizaka: ['iizaka', '도호쿠 · 후쿠시마현 · 이이자카', 'tohoku', 'fukushima', 'fukushima', 'iizaka'],
};
...
function newest(files, pattern) {
  return files.filter((file) => pattern.test(file)).sort().at(-1) ?? null;
}
...
function normalizePlatform(value) {
  ...
  if (lower.includes('nifty')) return '니프티 온천';
  if (lower.includes('jtb')) return 'JTB';
  if (lower.includes('4travel')) return '포트래블';
  if (lower.includes('forum')) return '일본어 포럼';
  return raw;
}
...
function signalPairs(row) {
  const paired = splitList(row.signal_tags).map((item) => {
  ...
  if (paired.length > 0) return paired.filter((item) => item.signal);
...
function loadBundle(slug) {
  ...
  const sampleFile =
    ...
    newest(files, /^combined_review_sample_.*\.csv$/) ??
    newest(files, /^.*direct_review_compact_.*\.csv$/);
  ...
  const aggregateFile = newest(files, /^signal_aggregate_.*\.json$/);
  const aggregate = aggregateFile ? readJson(path.join(dir, aggregateFile)) : null;
  const aggregateSignals = Array.isArray(aggregate?.signals) ? aggregate.signals : [];
...
function aggregateItems(bundle) {
  if (bundle.aggregateSignals.length > 0) {
    const aggregateItems = aggregateItemsFromSignals(bundle);
    if (aggregateItems.length >= 3) return aggregateItems;
  }
  ...
  for (const row of bundle.rows) {
    ...
    if (pairs.length === 0 && isTruthy(row.onsen_related ?? row.is_onsen_related)) pairs = deriveSignalsFromBathAreas(rawAreas);
...
function aggregateItemsFromSignals(bundle) {
  return bundle.aggregateSignals
    .map((row) => {
      const signal = normalizeSignal(row.signal_type ?? row.signal);
      const rawAreas = splitList(row.bath_area ?? row.bathArea);
      const mentions = toInt(row.mention_count ?? row.mentions ?? row.source_count);
      const platformCount = Math.min(toInt(row.platform_count ?? row.platforms_count) || bundle.platformCount, bundle.platformCount || toInt(row.platform_count));
      const direction = normalizeDirection(row.signal_direction ?? row.direction, signal);
      const directionCounts = directionCountsForAggregate(row, mentions, direction);
      const denominator = signalScope[signal] ?? 'onsen_related';
      const denominatorValue = denominator === 'experiences_read' ? bundle.directCount : bundle.onsenCount;
      const platforms = ensurePlatforms(
        Array.isArray(row.platforms) ? row.platforms.map(normalizePlatform).filter(Boolean) : bundle.platforms.slice(0, platformCount),
        platformCount
      );
      return {
        bathArea: selectBathArea(signal, rawAreas),
        signal,
        denominator,
        denominatorValue,
        mentions,
        ...
        platformCount,
        directionCounts,
        direction: dominantDirection(directionCounts),
...
function directionCountsForAggregate(row, mentions, fallbackDirection) {
  const source = row.direction_counts ?? row.directions;
  const counts =
    source && typeof source === 'object'
      ? {
          positive: toInt(source.positive),
          mixed: toInt(source.mixed),
          negative: toInt(source.negative),
          neutral: 0,
        }
      : {
          positive: fallbackDirection === 'positive' ? mentions : 0,
          mixed: fallbackDirection === 'mixed' ? mentions : 0,
          negative: fallbackDirection === 'negative' ? mentions : 0,
          neutral: 0,
        };
  const directionalSum = counts.positive + counts.mixed + counts.negative;
  if (directionalSum < mentions) {
    const remainder = mentions - directionalSum;
    if (fallbackDirection === 'positive') counts.positive += remainder;
    else if (fallbackDirection === 'negative') counts.negative += remainder;
    else counts.mixed += remainder;
  }
  return counts;
}
...
function waterCriteria(items, candidate) {
  const values = new Set(['spring_confirmed']);
  const official = `${candidate.official_or_product_fact_seen ?? candidate.initial_bath_signals ?? ''} ${candidate.status_basis ?? ''}`;
  if (/源泉かけ流し|源泉掛け流し|원천가케나가시|源泉100/.test(official)) values.add('direct_source');
...
function createAccommodation(candidate, bundle, items, level) {
  const meta = areaMeta[candidate.area_slug] ?? areaMeta.atami;
  const [region, area, regionGroup, prefecture, city, onsenArea] = meta;
  const bath = primaryBath(items);
  const identity = identityOverrides[candidate.slug] ?? {};
  const displayName = identity.displayNameKo ?? displayNameKo[candidate.slug] ?? candidate.name_ko_or_en ?? candidate.name_ja ?? candidate.slug;
  const nameJa = identity.nameJa ?? bundle.nameJa ?? candidate.name_ja ?? null;
  const nameEn = identity.nameEn ?? (/[A-Za-z]/.test(bundle.nameKoOrEn ?? candidate.name_ko_or_en ?? '') ? bundle.nameKoOrEn ?? candidate.name_ko_or_en : null);
  ...
  return {
    ...
    name_source_note:
      identity.nameSourceNote ??
      `${regionGroup === 'tohoku' ? '도호쿠' : '이즈/시즈오카'} 판정 seed 생성 과정에서 후보명과 딥리서치 산출물을 대조했습니다.`,
    region,
    area,
    country: 'JP',
    ...
    water_criteria: waterCriteria(items, candidate),
    summary: `${area}의 온천 숙소입니다. ${bath}으로 이용 경험이 반복되며, 객실 타입과 운영 조건을 예약 전에 함께 살펴보시기 바랍니다.`,
    primary_bath: bath,
    water_use_status: 'review_supported',
    water_source_type: waterCriteria(items, candidate).includes('direct_source') ? 'free_flowing_source' : 'hot_spring_confirmed',
    ...
    fact_statuses: [
      {
        ...
        source: candidate.source_urls?.split(';')[0]?.trim() || null,
      },
    ],
    status: 'published',
...
function validate(accommodations, verdicts) {
  const errors = [];
...
function createBacklog(rows, seededSlugs, missingStats) {
  const headers = ['scope', 'slug', 'name_ja', 'area_slug', 'candidate_status', 'backlog_bucket', 'next_action'];
  const body = rows
    .filter((row) => allowedStatuses.test(row.candidate_status ?? row.initial_tier ?? '') && !seededSlugs.has(row.slug))
    .map((row) => ({
      scope: tohokuPrefectures.has(row.prefecture) ? 'tohoku' : 'izu_shizuoka',
      slug: row.slug,
      name_ja: row.name_ja,
      area_slug: row.area_slug,
      candidate_status: row.candidate_status ?? row.initial_tier,
      backlog_bucket: missingStats.has(row.slug) ? '딥리서치 원천 없음' : '판정 기준 미달',
      next_action: missingStats.has(row.slug)
        ? '숙소별 구조화 표본을 확보한 뒤 판정 seed를 재생성합니다.'
        : '직접 수, 온천 관련 수, 플랫폼 수, 채택 item 수를 보강합니다.',
    }));
  return `${headers.join(',')}\n${body.map((row) => headers.map((header) => csvEscape(row[header])).join(',')).join('\n')}\n`;
}
...
function createReport(reportRows, backlogRows) {
  ...
  const lines = [
    ...
    `- backlog: ${backlogRows.length}곳`,
    '',
    '## Seed 대상',
    '',
    '| scope | slug | 숙소명 | direct | onsen | platforms | verdict | items | source |',
    '|---|---|---|---:|---:|---:|---|---:|---|',
    ...reportRows.map((row) => `| ${row.scope} | ${row.slug} | ${row.name} | ${row.direct} | ${row.onsen} | ${row.platforms} | ${row.level} | ${row.items} | ${row.source} |`),
    '',
    '## 산출물',
    '',
    ...
    '- 기준 미달 숙소는 active accommodation과 lite verdict로 생성하되, items는 비워 DB 노출에서 과도한 근거화를 피했다.',
    '- 검색 스니펫, OTA 요약, 공급자 카드는 원천 sample CSV에 직접 본문으로 들어온 경우가 아니면 판정 item 집계에 쓰지 않았다.',
    '',
  ];
  return `${lines.join('\n')}\n`;
}
...
function collectCandidates() {
  const izuRows = readCsv(izuCandidatePath).filter((row) => row.kind === 'accommodation');
  const tohokuRows = readCsv(nationwideMasterPath)
    .filter((row) => tohokuPrefectures.has(row.prefecture))
    .map((row) => ({
      ...row,
      kind: 'accommodation',
      type: row.property_type,
      candidate_status: row.initial_tier === 'Tier 1' ? 'ready' : 'hold',
      official_or_product_fact_seen: row.initial_bath_signals,
      source_urls: '',
    }));
  return [...izuRows, ...tohokuRows];
}
...
async function main() {
  ...
  for (const candidate of candidates) {
    if (!allowedStatuses.test(candidate.candidate_status ?? '')) continue;
    if (!areaMeta[candidate.area_slug]) continue;
    const bundle = loadBundle(candidate.slug);
    if (!bundle) {
      missingStats.add(candidate.slug);
      continue;
    }
    ...
    reportRows.push({
      scope: accommodation.region_group === 'tohoku' ? 'tohoku' : 'izu_shizuoka',
      slug: candidate.slug,
      name: accommodation.display_name_ko,
      direct: bundle.directCount,
      onsen: bundle.onsenCount,
      platforms: bundle.platformCount,
      level,
      items: verdict.items.length,
      source: `${bundle.statsFile} / ${bundle.sampleFile}`,
    });
  }

  const seededSlugs = new Set(accommodations.map((row) => row.slug));
  const backlogRows = candidates.filter((row) => allowedStatuses.test(row.candidate_status ?? '') && !seededSlugs.has(row.slug));
  const errors = validate(accommodations, verdicts);
  if (errors.length > 0) {
  ...
  await writeFile(outputBacklogPath, createBacklog(candidates, seededSlugs, missingStats));
  await writeFile(outputReportPath, createReport(reportRows, backlogRows));
