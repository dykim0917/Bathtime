...
const terms = [
  ...
  '濁り湯', '濁る', '引き湯', '源泉', 'かけ流し', '循環', '加水', '加温',
  '塩化物泉', '含鉄', '鉄分', '泉質', '湯質', '美肌', 'すべすべ', 'つるつる',
  '露天', '露天風呂', '客室露天', '客室露天風呂', '露天風呂付',
  '露天風呂付き', '露天風呂付客室', '温泉露天風呂付客室', '全客室',
  '部屋風呂', '客室風呂', 'お部屋のお風呂', '内風呂', '温泉付き客室',
  '金泉露天風呂付客室', '客室露天風呂', '金泉露天', '大浴場', '湯処', 'サウナ',
  '水風呂', '足湯', '貸切', '貸切風呂', '家族風呂', '展望風呂',
  '展望浴場', '屋上', '貸切露天', '貸切露天風呂', '展望貸切露天風呂',
  '展望金泉', '展望銀泉', 'ラウンジ', '有馬温泉街', '有馬',
  '温泉街', '有馬温泉駅', '送迎',
  'ぬるい', '熱い', '温度', '湯加減', '塩素', 'カルキ', '温泉感',
  '溜め湯', 'ため湯', 'かけ流しではない', '濁り湯', '茶褐色',
  '混雑', '混んで', '空いて', '静か', '眺望', '目隠し',
  '予約', '送迎', '駅', '徒歩', '館内', '古い', '歴史', '清潔', '掃除',
  '臭い', '匂い', '下水', '虫', '露天は狭い', '狭い', '小さい', 'コンパクト',
  '駐車場', '高い'
];
...
function tag({ platform, sourceUrl, id, title, body, roomType, planName, reviewDate, stayDate, score }) {
  ...
  if (/客室.*露天|部屋.*露天|お部屋.*露天|金泉露天.*客室|露天風呂付き|露天風呂付|露天.*客室|温泉付き客室|温泉露天風呂付客室/.test(combined)) {
  ...
  if (/大浴場|湯処|展望風呂|展望浴場|屋上.*風呂/.test(bodyText)) {
    bathAreas.add('public_bath');
    signals.add('public_bath_hot_spring');
  }
  if (/露天風呂|露天/.test(bodyText) && /大浴場|湯処|展望風呂|展望浴場|屋上|サウナ/.test(bodyText)) {
    bathAreas.add('open_air_public_bath');
    signals.add('public_bath_hot_spring');
  }
  if (/貸切風呂|貸切露天|展望貸切/.test(bodyText)) {
  ...
  if (/有馬温泉|金泉|銀泉|赤湯|赤褐色|茶褐色|含鉄|鉄分|引き湯|源泉|泉質|湯質|塩化物泉|美肌|すべすべ|つるつる|やわらか|柔らか|腰痛|濁り湯|濁る/.test(bodyText)) {
    signals.add('water_texture');
  }
  if (/温泉感|普通のお湯|薄い|循環|塩素|カルキ|溜め湯|ため湯|かけ流しではない/.test(bodyText)) signals.add('weak_onsen_feeling');
  if (/塩素|カルキ/.test(bodyText)) signals.add('chlorine_smell');
  if (/混雑|混んで|空いて|静か|待ち|貸切状態|人も少なく|人が少な/.test(bodyText)) signals.add('crowding');
  if (/予約|送迎|駅|徒歩|有馬温泉街|外湯|説明|チェックイン|案内|部屋違い|館内|遠い|階段|タクシー/.test(bodyText)) {
  ...
  for (const [name, pattern] of [
    ...
    ['insects', /虫|蚊/],
    ['arima_view_location', /眺望|目隠し|景色|有馬|展望|有馬温泉街|六甲|山/],
    ['access_booking', /予約|送迎|駅|徒歩|有馬温泉駅|有馬温泉街|温泉街|チェックイン|館内|遠い|坂|階段|タクシー|駐車場/],
    ['public_bath_size_expectation', /大浴場.*小さ|大浴場.*狭|コンパクト|小さい/],
    ['lounge_operation', /ラウンジ|スカイラウンジ|フリードリンク|アイス|コーヒー牛乳/],
  ...
  return {
    ...
    onsen_related_body: bodyKeywords.some((kw) => /温泉|風呂|湯|金泉|銀泉|赤湯|源泉|泉質|サウナ|貸切|家族風呂|足湯|外湯|入浴/.test(kw)),
...
async function collectRakuten(maxPages = 18) {
  ...
  for (let page = 1; page <= maxPages; page++) {
    const url = page === 1
      ? 'https://travel.rakuten.co.jp/HOTEL/9576/review.html'
      : `https://review.travel.rakuten.co.jp/hotel/voice/9576?page=${page}`;
    ...
    for (const item of contents) {
      ...
      reviews.push(tag({
        ...
        sourceUrl: 'https://travel.rakuten.co.jp/HOTEL/9576/review.html',
        id: item.id,
        title: item.title,
        body: item.comment,
...
function parseJalanBlock(block, sourceUrl) {
  ...
  return tag({
    ...
    roomType: text.match(/(和室|和洋室|ツイン|露天風呂付|温泉付き客室|海側|有馬温泉街|スイート|トップフロア)/)?.[1] || '',
...
async function collectJalan() {
  ...
  const urls = [0, 30, 60, 90, 120, 150].map((idx) => idx === 0
    ? 'https://www.jalan.net/yad349968/kuchikomi/'
    : `https://www.jalan.net/yad349968/kuchikomi/?screenId=UWW3701&idx=${idx}`);
...
async function collectJtb(maxPages = 8) {
  ...
  for (let page = 1; page <= maxPages; page++) {
    const url = 'https://www.jtb.co.jp/kokunai-hotel/htl/6435022/review/?area=38&room=1&dateunspecified=1&staynight=1&roomassign=m2'
      + (page > 1 ? `&page=${page}` : '');
    const html = await fetchHtml(url);
...
await fs.writeFile(path.join(outDir, `hanamusubi_static_review_tags_${TODAY}.json`), JSON.stringify({ summary, reviews }, null, 2));
await fs.writeFile(path.join(outDir, `hanamusubi_static_review_tags_summary_${TODAY}.json`), JSON.stringify(summary, null, 2));
