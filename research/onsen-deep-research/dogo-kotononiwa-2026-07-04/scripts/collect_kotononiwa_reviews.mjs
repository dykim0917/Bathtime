...
const terms = [
  ...
  '家族風呂', '展望風呂', '展望浴場', '屋上', 'ラウンジ', 'スカイラウンジ',
  '道後本館', '道後温泉本館', '外湯', '商店街',
...
function tag({ platform, sourceUrl, id, title, body, roomType, planName, reviewDate, stayDate, score }) {
  ...
  for (const [name, pattern] of [
    ...
    ['insects', /虫|蚊/],
    ['view_garden', /眺望|庭園|中庭|目隠し|景色|夜景|松山城|展望/],
...
async function collectRakuten(maxPages = 18) {
  ...
  for (let page = 1; page <= maxPages; page++) {
    const url = page === 1
      ? 'https://travel.rakuten.co.jp/HOTEL/178753/review.html'
      : `https://review.travel.rakuten.co.jp/hotel/voice/178753?page=${page}`;
    ...
    for (const item of contents) {
      ...
      reviews.push(tag({
        ...
        sourceUrl: 'https://travel.rakuten.co.jp/HOTEL/178753/review.html',
        id: item.id,
        title: item.title,
        body: item.comment,
...
async function collectJalan() {
  ...
  const urls = [0, 30, 60, 90, 120, 150].map((idx) => idx === 0
    ? 'https://www.jalan.net/yad339801/kuchikomi/'
    : `https://www.jalan.net/yad339801/kuchikomi/?screenId=UWW3701&idx=${idx}`);
...
async function collectJtb(maxPages = 8) {
  ...
  for (let page = 1; page <= maxPages; page++) {
    const url = 'https://www.jtb.co.jp/kokunai-hotel/htl/7461A25/review/?area=38&room=1&dateunspecified=1&staynight=1&roomassign=m2'
      + (page > 1 ? `&page=${page}` : '');
    const html = await fetchHtml(url);
...
await fs.writeFile(path.join(outDir, `kotononiwa_static_review_tags_${TODAY}.json`), JSON.stringify({ summary, reviews }, null, 2));
await fs.writeFile(path.join(outDir, `kotononiwa_static_review_tags_summary_${TODAY}.json`), JSON.stringify(summary, null, 2));
