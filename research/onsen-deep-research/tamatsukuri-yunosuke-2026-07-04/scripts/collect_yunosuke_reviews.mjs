...
const RAKUTEN_ID = 10743;
const JALAN_ID = 306939;
const JTB_ID = '7323006';

const terms = [
  '温泉', '玉造温泉', '長生閣', '源泉', '自家源泉', '泉質', '湯質',
  '単純温泉', '低張性弱アルカリ性泉', 'アルカリ性単純泉',
  '美肌', '美肌の湯', 'つるつる', 'ツルツル', 'すべすべ', '肌触り',
  '大浴場', 'めのう風呂', 'メノウ風呂', '瑪瑙', '勾玉', '勾玉橋',
  '露天', '露天風呂', '大きな石の露天風呂', '展望風呂',
...
function tag({ platform, sourceUrl, id, title, body, roomType, planName, reviewDate, stayDate, score, language = 'ja' }) {
  ...
  if (/大浴場|めのう風呂|メノウ風呂|瑪瑙|浴場|お風呂|風呂/.test(bodyText)) {
    bathAreas.add('public_bath');
    signals.add('public_bath_hot_spring');
  }
  if (/露天風呂|大きな石の露天風呂/.test(bodyText) && !/客室|部屋|お部屋/.test(bodyText)) {
  ...
  if (/温泉|玉造温泉|源泉|自家源泉|泉質|湯質|美肌|美肌の湯|アルカリ性|肌触り|つるつる|ツルツル|すべすべ|温ま/.test(bodyText)) {
  ...
  for (let offset = 0; offset < maxReviews; offset += 20) {
    ...
    for (const item of data.contents || []) {
      ...
      reviews.push(tag({
        ...
        sourceUrl: 'https://travel.rakuten.co.jp/HOTEL/10743/review.html',
        id: item.id,
        title: item.title,
        body: item.comment,
...
function parseJalanBlock(block, sourceUrl) {
  ...
  let body = text
    .replace(/玉造グランドホテル 長生閣からの返信[\s\S]*$/, '')
    .replace(/返信日：[\s\S]*$/, '');
...
await fs.writeFile(path.join(outDir, `yunosuke_static_review_tags_${TODAY}.json`), JSON.stringify(rows, null, 2));
await fs.writeFile(path.join(outDir, `yunosuke_static_review_tags_summary_${TODAY}.json`), JSON.stringify(summary, null, 2));
