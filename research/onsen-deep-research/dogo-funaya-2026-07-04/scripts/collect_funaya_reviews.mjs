...
const terms = [
  ...
  '露天', '露天風呂', '客室露天', '露天風呂付', '部屋風呂', '客室風呂',
  '内風呂', '温泉付き客室', '大浴場', '御影湯', '檜湯', '湯屋', 'サウナ',
  '水風呂', '足湯', '貸切', '貸切風呂', '家族風呂', '道後本館', '外湯',
  'ぬるい', '熱い', '温度', '湯加減', '塩素', 'カルキ', '温泉感',
  '混雑', '混んで', '空いて', '静か', '眺望', '庭園', '中庭', '川',
...
function tag({ platform, sourceUrl, id, title, body, roomType, planName, reviewDate, stayDate, score }) {
  ...
  if (/客室.*露天|部屋.*露天|露天風呂付き|露天風呂付|露天.*客室|温泉付き客室/.test(combined)) {
  ...
  if (/大浴場|御影湯|檜湯|湯屋/.test(bodyText)) {
    bathAreas.add('public_bath');
    signals.add('public_bath_hot_spring');
  }
  if (/露天風呂|露天/.test(bodyText) && /大浴場|御影湯|檜湯|湯屋|サウナ/.test(bodyText)) {
  ...
  if (/足湯/.test(bodyText)) bathAreas.add('facility_wide');
  if (/道後温泉|引き湯|源泉|泉質|湯質|アルカリ|単純泉|美肌|すべすべ|つるつる|腰痛/.test(bodyText)) {
  ...
  for (const [name, pattern] of [
    ...
    ['insects', /虫|蚊/],
    ['view_garden', /眺望|庭園|中庭|川|景色/],
    ['access_booking', /予約|送迎|駅|徒歩|道後本館|外湯|チェックイン|館内|遠い|階段|タクシー/]
  ...
  return {
    ...
    onsen_related_body: bodyKeywords.some((kw) => /温泉|風呂|湯|源泉|泉質|サウナ|貸切|家族風呂|足湯|道後本館|外湯|入浴/.test(kw)),
...
async function collectRakuten(maxPages = 18) {
  ...
  for (let page = 1; page <= maxPages; page++) {
    const url = page === 1
      ? 'https://travel.rakuten.co.jp/HOTEL/11332/review.html'
      : `https://review.travel.rakuten.co.jp/hotel/voice/11332?page=${page}`;
    ...
    for (const item of contents) {
      ...
      reviews.push(tag({
        ...
        sourceUrl: 'https://travel.rakuten.co.jp/HOTEL/11332/review.html',
        id: item.id,
        title: item.title,
        body: item.comment,
...
async function collectJalan() {
  ...
  const urls = [0, 30, 60, 90, 120, 150].map((idx) => idx === 0
    ? 'https://www.jalan.net/yad356524/kuchikomi/'
    : `https://www.jalan.net/yad356524/kuchikomi/?screenId=UWW3701&idx=${idx}`);
...
async function collectJtb(maxPages = 8) {
  ...
  for (let page = 1; page <= maxPages; page++) {
    const url = 'https://www.jtb.co.jp/kokunai-hotel/htl/7461008/review/?area=38&room=1&dateunspecified=1&staynight=1&roomassign=m2'
      + (page > 1 ? `&page=${page}` : '');
    const html = await fetchHtml(url);
...
await fs.writeFile(path.join(outDir, `funaya_static_review_tags_${TODAY}.json`), JSON.stringify({ summary, reviews }, null, 2));
await fs.writeFile(path.join(outDir, `funaya_static_review_tags_summary_${TODAY}.json`), JSON.stringify(summary, null, 2));
