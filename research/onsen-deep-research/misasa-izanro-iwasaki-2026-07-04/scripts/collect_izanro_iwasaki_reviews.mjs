...
const RAKUTEN_ID = 5840;
const JALAN_ID = 347748;

const terms = [
  '温泉', '三朝温泉', 'ラジウム', 'ラドン', '源泉', '源泉掛け流し', '源泉かけ流し', '掛け流し', 'かけ流し',
  '泉質', '湯質', '飲泉', '飲泉場', '蒸気', '吸入', 'ミスト', '蒸し風呂',
  '大浴場', '庭園風呂', '回遊式', '山の湯', '右の湯', '左の湯', '露天', '露天風呂', '内湯',
  '貸切', '貸切風呂', '家族風呂', '客室露天', '半露天', '部屋風呂', '客室風呂',
  'ぬるい', '熱い', '温度', '湯加減', '塩素', 'カルキ', '温泉感', '効能', '湯治',
  '混雑', '混んで', '人が多い', '空いて', '貸切状態',
  '古い', '老朽', '昭和', '清潔', '掃除', '臭い', '匂い', 'カビ', '虫',
  '川', '眺望', '景色', '庭', '広い', '種類', 'たくさん', '迷路',
  '予約', '送迎', '駐車場', '案内', '階段', 'バリアフリー', '部屋食'
];
...
function tag({ platform, sourceUrl, id, title, body, roomType, planName, reviewDate, stayDate, score, language = 'ja' }) {
  ...
  if (/大浴場|庭園風呂|回遊式|山の湯|右の湯|左の湯/.test(combined)) {
    bathAreas.add('public_bath');
    signals.add('public_bath_hot_spring');
  }
  if (/露天風呂|露天/.test(combined) && !/客室露天|部屋.*露天|半露天/.test(combined)) {
    bathAreas.add('open_air_public_bath');
    signals.add('public_bath_hot_spring');
  }
  if (/客室露天|部屋.*露天|半露天/.test(combined)) {
    bathAreas.add('room_open_air_bath');
    signals.add('room_bath_hot_spring');
  }
  if (/部屋風呂|客室風呂|内湯/.test(combined) && /部屋|客室/.test(combined)) {
    bathAreas.add('room_bath');
    signals.add('room_bath_hot_spring');
  }
  if (/貸切風呂|貸切露天/.test(bodyText) && !/部屋|客室/.test(bodyText)) {
  ...
  if (/温泉|三朝温泉|ラジウム|ラドン|源泉|掛け流し|かけ流し|泉質|湯質|飲泉|蒸気|吸入|湯治|効能/.test(bodyText)) {
  ...
  if (/予約|送迎|駐車場|説明|案内|チェックイン|階段|部屋食|バリアフリー/.test(bodyText)) signals.add('booking_confusion');

  for (const [name, pattern] of [
    ['temperature_control', /ぬるい|熱い|温度|湯加減/],
    ['cleanliness_aging', /古い|老朽|昭和|掃除|臭い|匂い|汚|清潔|カビ/],
    ['insects', /虫|蚊|カメムシ/],
    ['view_garden_river', /川|眺望|景色|庭|広い/],
    ['bath_variety_layout', /種類|たくさん|迷路|回遊式|山の湯|右の湯|左の湯/],
    ['radium_health_framing', /ラジウム|ラドン|飲泉|蒸気|吸入|湯治|効能/],
    ['access_booking', /予約|送迎|駐車場|チェックイン|階段|案内|バリアフリー/],
    ['room_bath_presence', /客室露天|半露天|部屋風呂|客室風呂/]
  ...
  return {
    ...
    onsen_related_body: bodyKeywords.some((kw) => /温泉|湯|源泉|泉質|露天|風呂|ラジウム|ラドン|飲泉|吸入|大浴場/.test(kw)),
  ...
  for (let offset = 0; offset < maxReviews; offset += 20) {
    ...
    for (const item of data.contents || []) {
      ...
      reviews.push(tag({
        ...
        sourceUrl: 'https://travel.rakuten.co.jp/HOTEL/5840/review.html',
        id: item.id,
        title: item.title,
        body: item.comment,
...
function parseJalanBlock(block, sourceUrl) {
  ...
  let body = text
    .replace(/三朝温泉　依山楼岩崎.*?からの返信[\s\S]*$/, '')
    .replace(/依山楼岩崎.*?からの返信[\s\S]*$/, '')
    .replace(/返信日：[\s\S]*$/, '');
...
async function collectJalan(maxPages = 18) {
  const reviews = [];
  const stats = {};
  const seen = new Set();
  let visibleTotal = null;
...
async function main() {
  ...
  await fs.writeFile(path.join(outDir, `izanro_iwasaki_static_review_tags_${TODAY}.json`), JSON.stringify(rows, null, 2));
  await fs.writeFile(path.join(outDir, `izanro_iwasaki_static_review_tags_summary_${TODAY}.json`), JSON.stringify(summary, null, 2));
