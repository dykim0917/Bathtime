...
function tag({ platform, sourceUrl, id, title, body, roomType, planName, reviewDate, stayDate, score }) {
  ...
  if (/客室.*露天|部屋.*露天|お部屋.*露天|テラス.*露天|露天風呂付き|露天風呂付|露天.*客室|温泉付き客室|温泉露天風呂付客室/.test(combined)) {
  ...
  if (/大浴場|湯処|展望風呂|展望浴場|屋上.*風呂|天空露天風呂|インフィニティ/.test(bodyText)) {
    bathAreas.add('public_bath');
    signals.add('public_bath_hot_spring');
  }
  if (/露天風呂|露天|インフィニティ|天空/.test(bodyText) && /大浴場|湯処|展望風呂|展望浴場|屋上|サウナ|天空|インフィニティ/.test(bodyText)) {
  ...
  if (/皆生温泉|引き湯|源泉|泉質|湯質|アルカリ|単純泉|美肌|すべすべ|つるつる|やわらか|柔らか|腰痛|濁り湯|茶褐色/.test(bodyText)) {
  ...
  if (/予約|送迎|駅|徒歩|皆生海岸|外湯|説明|チェックイン|案内|部屋違い|館内|遠い|階段|タクシー/.test(bodyText)) {
  ...
  for (const [name, pattern] of [
    ...
    ['insects', /虫|蚊/],
    ['sea_view', /眺望|目隠し|景色|日本海|海|波|夕日|朝日|展望|オーシャンビュー/],
    ['access_booking', /予約|送迎|駅|徒歩|皆生海岸|海水浴場|海岸|チェックイン|館内|遠い|階段|タクシー|駐車場/],
...
async function collectRakuten(maxPages = 18) {
  ...
  for (let page = 1; page <= maxPages; page++) {
    const url = page === 1
      ? 'https://travel.rakuten.co.jp/HOTEL/168732/review.html'
      : `https://review.travel.rakuten.co.jp/hotel/voice/168732?page=${page}`;
    ...
    for (const item of contents) {
      ...
      reviews.push(tag({
        ...
        sourceUrl: 'https://travel.rakuten.co.jp/HOTEL/168732/review.html',
        id: item.id,
        title: item.title,
        body: item.comment,
...
function parseJalanBlock(block, sourceUrl) {
  ...
  return tag({
    ...
    roomType: text.match(/(和室|和洋室|ツイン|露天風呂付|温泉付き客室|海側|オーシャンビュー|スイート|トップフロア)/)?.[1] || '',
...
async function collectJalan() {
  ...
  const urls = [0, 30, 60, 90, 120, 150].map((idx) => idx === 0
    ? 'https://www.jalan.net/yad398764/kuchikomi/'
    : `https://www.jalan.net/yad398764/kuchikomi/?screenId=UWW3701&idx=${idx}`);
...
async function collectJtb(maxPages = 8) {
  ...
  for (let page = 1; page <= maxPages; page++) {
    const url = 'https://www.jtb.co.jp/kokunai-hotel/htl/7316032/review/?area=38&room=1&dateunspecified=1&staynight=1&roomassign=m2'
      + (page > 1 ? `&page=${page}` : '');
    const html = await fetchHtml(url);
...
await fs.writeFile(path.join(outDir, `yugetsu_static_review_tags_${TODAY}.json`), JSON.stringify({ summary, reviews }, null, 2));
await fs.writeFile(path.join(outDir, `yugetsu_static_review_tags_summary_${TODAY}.json`), JSON.stringify(summary, null, 2));
