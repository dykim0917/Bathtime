...
const RAKUTEN_ID = 8226;
const JALAN_ID = 316623;
const JTB_ID = '6506008';

const terms = [
  '温泉', '白浜温泉', '源泉', 'かけ流し', '源泉かけ流し', '掛け流し',
  '藤の湯', '衝幹の湯', '湯の花', '泉質', '湯質', '美肌', 'すべすべ',
  '露天', '露天風呂', '客室露天', '客室露天風呂', '露天風呂付',
  '露天風呂付き', '露天風呂付客室', '温泉露天風呂', '温泉付き客室',
  '部屋風呂', '客室風呂', '部屋付き露天風呂', '部屋の露天風呂',
  '大浴場', '内湯', '浴場', '貸切', '貸切風呂', '家族風呂',
  'ミラバス', '白良浜', 'オーシャンビュー', '海', '眺望', '景色',
  'ぬるい', '熱い', '温度', '湯加減', '塩素', 'カルキ', '温泉感',
  '混雑', '混んで', '空いて', '静か', '待ち', '入替', '男女入替',
  '清潔', '掃除', '古い', '臭い', '匂い', '虫', '狭い', '広い',
  '子連れ', '赤ちゃん', 'バリアフリー', '高い', '送迎', '駐車場'
];
...
function tag({ platform, sourceUrl, id, title, body, roomType, planName, reviewDate, stayDate, score, language = 'ja' }) {
  ...
  if (/客室.*露天|部屋.*露天|部屋付き露天|お部屋.*露天|露天風呂付き|露天風呂付|露天.*客室|温泉付き客室|温泉露天風呂/.test(combined)) {
    bathAreas.add('room_open_air_bath');
    signals.add('room_bath_hot_spring');
  }
  if (/部屋風呂|客室風呂|部屋の風呂|内風呂|内湯|部屋.*風呂|ミラバス/.test(combined)) {
    bathAreas.add('room_bath');
    signals.add('room_bath_hot_spring');
  }
  if (/大浴場|浴場|藤の湯|衝幹の湯/.test(bodyText)) {
    bathAreas.add('public_bath');
    signals.add('public_bath_hot_spring');
  }
  if (/露天風呂|露天/.test(bodyText) && /大浴場|浴場|共同|男女|入替/.test(bodyText)) {
  ...
  if (/白浜温泉|源泉|かけ流し|掛け流し|湯の花|藤の湯|衝幹の湯|泉質|湯質|美肌|すべすべ|つるつる|保温|温ま/.test(bodyText)) {
    signals.add('water_texture');
  }
  if (/温泉感|普通のお湯|薄い|循環|塩素|カルキ|源泉かけ流しではない/.test(bodyText)) signals.add('weak_onsen_feeling');
  if (/塩素|カルキ/.test(bodyText)) signals.add('chlorine_smell');
  if (/混雑|混んで|空いて|静か|待ち|貸切状態|人も少なく|人が少な/.test(bodyText)) signals.add('crowding');
  if (/予約|送迎|駅|駐車場|説明|チェックイン|案内|部屋違い|館内|階段|入替|男女入替/.test(bodyText)) signals.add('booking_confusion');

  for (const [name, pattern] of [
    ['temperature_control', /ぬるい|熱い|温度|湯加減/],
    ['cleanliness_aging', /掃除|古い|臭い|匂い|汚|老朽|清潔/],
    ['insects', /虫|蚊/],
    ['view_privacy', /眺望|景色|白良浜|海|オーシャンビュー|目隠し|見え/],
    ['access_booking', /予約|送迎|駅|駐車場|チェックイン|館内|階段|電話|入替/],
    ['public_bath_size_expectation', /大浴場.*小さ|大浴場.*狭|狭い|広い/],
  ...
  return {
    ...
    plan_name: norm(planName).slice(0, 160),
    body_char_count: bodyText.length,
    body_keywords: [...new Set(bodyKeywords)],
    context_keywords: [...new Set(contextKeywords)],
    onsen_related_body: bodyKeywords.some((kw) => /温泉|風呂|湯|源泉|泉質|露天|大浴場|ミラバス|入浴/.test(kw)),
  ...
  for (let offset = 0; offset < maxReviews; offset += 20) {
    ...
    for (const item of data.contents || []) {
      ...
      reviews.push(tag({
        ...
        sourceUrl: 'https://travel.rakuten.co.jp/HOTEL/8226/review.html',
        id: item.id,
        title: item.title,
        body: item.comment,
...
function parseJalanBlock(block, sourceUrl) {
  ...
  return tag({
    ...
    roomType: text.match(/(露天風呂付|温泉露天風呂|ミラバス|和室|和洋室|洋室|ましらの|浜水晶|波の綾)/)?.[1] || '',
    planName: text.match(/プラン名\s*(.*?)\s*(部屋タイプ|和室|洋室|投稿日|時期)/)?.[1] || ''
  });
}

async function collectJalan(maxPages = 6) {
  const reviews = [];
  const stats = {};
  const seen = new Set();
  let visibleTotal = null;
...
const jalan = await collectJalan(6);
...
await fs.writeFile(path.join(outDir, `sanrakuso_static_review_tags_${TODAY}.json`), JSON.stringify(rows, null, 2));
await fs.writeFile(path.join(outDir, `sanrakuso_static_review_tags_summary_${TODAY}.json`), JSON.stringify(summary, null, 2));
