...
const RAKUTEN_ID = 14007;
const JALAN_ID = 332274;
const JTB_ID = '6318001';

const terms = [
  '温泉', '城崎温泉', '西村屋ホテル招月庭', '西村屋', '招月庭', '源泉', '泉質', '湯質',
  'ナトリウム・カルシウム-塩化物泉', '塩化物泉', '沸かし湯', '白湯',
  '美肌', '美肌の湯', 'つるつる', 'ツルツル', 'すべすべ', '肌触り',
  '大浴場', '月下の湯', '内湯', 'ジャグジーバス', 'ミストサウナ',
  '露天', '露天風呂', '露天風呂エリア', '展望風呂', '外湯', '外湯めぐり', '外湯巡り',
  '客室露天', '露天風呂付き', '露天風呂付', '部屋風呂', '客室風呂', '内湯',
  '貸切', '貸切風呂', '貸切露天風呂', '森のプライベートスパ', '家族風呂', 'サウナ', 'ぬるい', '熱い', '温度', '湯加減',
  '塩素', 'カルキ', '温泉感', '混雑', '混んで', '空いて', '待ち',
  '予約', '説明', '案内', '清潔', '掃除', '古い', '老朽', '臭い', '匂い',
  '虫', '狭い', '広い', '眺望', '庭', '日本庭園', '駅', '送迎', '駐車場',
  'バイキング', '子連れ', '赤ちゃん', '家族'
];
...
function tag({ platform, sourceUrl, id, title, body, roomType, planName, reviewDate, stayDate, score, language = 'ja' }) {
  ...
  if (/大浴場|月下の湯|浴場|お風呂|風呂|内湯|ジャグジーバス|ミストサウナ/.test(bodyText)) {
    bathAreas.add('public_bath');
    signals.add('public_bath_hot_spring');
  }
  if (/露天風呂|露天風呂エリア/.test(bodyText) && !/客室|部屋|お部屋/.test(bodyText)) {
    bathAreas.add('open_air_public_bath');
    signals.add('public_bath_hot_spring');
  }
  if (/貸切風呂|貸切露天風呂|貸切|森のプライベートスパ/.test(bodyText)) {
  ...
  if (/温泉|城崎温泉|源泉|泉質|湯質|美肌|美肌の湯|塩化物泉|肌触り|つるつる|ツルツル|すべすべ|温ま|外湯/.test(bodyText)) {
    signals.add('water_texture');
  }
  if (/温泉感|普通のお湯|薄い|水道水|沸かし湯|白湯|温泉ではない|ツルツルしない/.test(bodyText)) signals.add('weak_onsen_feeling');
  ...
  for (const [name, pattern] of [
    ...
    ['insects', /虫|蚊|カメムシ/],
    ['view_privacy', /眺望|景色|庭|日本庭園|見え|展望/],
    ['access_booking', /予約|送迎|駅|駐車場|チェックイン|館内|電話|鍵|説明|案内/],
    ['public_bath_size_expectation', /大浴場.*小さ|大浴場.*狭|狭い|広い|お風呂.*広|風呂.*広/],
  ...
  for (let offset = 0; offset < maxReviews; offset += 20) {
    ...
    for (const item of data.contents || []) {
      ...
      reviews.push(tag({
        ...
        sourceUrl: 'https://travel.rakuten.co.jp/HOTEL/14007/review.html',
        id: item.id,
        title: item.title,
        body: item.comment,
...
function parseJalanBlock(block, sourceUrl) {
  ...
  return tag({
    ...
    roomType: text.match(/(露天風呂付|露天風呂付き|温泉露天風呂|和室|和洋室|洋室|ツイン|月の棟|庭の棟|客室露天)/)?.[1] || '',
    planName: text.match(/プラン\s*(.*?)\s*(価格帯|和室|和洋室|洋室|ツイン|その他|投稿日|部屋)/)?.[1] || ''
  });
}
...
await fs.writeFile(path.join(outDir, `shogetsutei_static_review_tags_${TODAY}.json`), JSON.stringify(rows, null, 2));
await fs.writeFile(path.join(outDir, `shogetsutei_static_review_tags_summary_${TODAY}.json`), JSON.stringify(summary, null, 2));
