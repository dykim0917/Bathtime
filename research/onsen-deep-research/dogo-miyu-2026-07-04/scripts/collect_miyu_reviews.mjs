...
async function collectRakuten(maxPages = 18) {
  ...
  for (let page = 1; page <= maxPages; page++) {
    const url = page === 1
      ? 'https://travel.rakuten.co.jp/HOTEL/165194/review.html'
      : `https://review.travel.rakuten.co.jp/hotel/voice/165194?page=${page}`;
    ...
    for (const item of contents) {
      ...
      reviews.push(tag({
        ...
        sourceUrl: 'https://travel.rakuten.co.jp/HOTEL/165194/review.html',
        id: item.id,
        title: item.title,
        body: item.comment,
...
async function collectJalan() {
  ...
  const urls = [0, 30, 60, 90, 120, 150].map((idx) => idx === 0
    ? 'https://www.jalan.net/yad381624/kuchikomi/'
    : `https://www.jalan.net/yad381624/kuchikomi/?screenId=UWW3701&idx=${idx}`);
...
async function collectJtb(maxPages = 8) {
  ...
  for (let page = 1; page <= maxPages; page++) {
    const url = 'https://www.jtb.co.jp/kokunai-hotel/htl/7461004/review/?area=38&room=1&dateunspecified=1&staynight=1&roomassign=m2'
      + (page > 1 ? `&page=${page}` : '');
    const html = await fetchHtml(url);
...
await fs.writeFile(path.join(outDir, `miyu_static_review_tags_${TODAY}.json`), JSON.stringify({ summary, reviews }, null, 2));
await fs.writeFile(path.join(outDir, `miyu_static_review_tags_summary_${TODAY}.json`), JSON.stringify(summary, null, 2));
