...
const RAKUTEN_ID = 4761;
const JALAN_ID = 322346;
const JTB_ID = '6112005';

const terms = [
  '温泉', '鳥羽温泉郷', '榊原温泉', '七栗の湯', '源泉', '泉質', '湯質',
  'アルカリ性単純泉', 'つるつる', 'ツルツル', 'すべすべ', '肌触り',
  '大浴場', '嬉春亭大浴場', '南館大浴場', '温泉村', '湯亭', '湯めぐり',
  '露天', '露天風呂', '野天風呂', '風流野天風呂', '客室露天',
  '露天風呂付き', '露天風呂付', '部屋風呂', '客室風呂', '内湯',
  '貸切', '貸切風呂', '無料貸切風呂', '家族風呂', '風水家族風呂',
  '貝殻風呂', 'しゃこ貝風呂', '浮世風呂', '瓶風呂', '釜風呂', 'たぬき風呂',
  '足湯', '岩盤浴', 'サウナ', 'ぬるい', '熱い', '温度', '湯加減',
  '塩素', 'カルキ', '温泉感', '混雑', '混んで', '空いて', '待ち',
  '予約', '不要', '鍵', '時間制限', '清潔', '掃除', '古い', '老朽',
  '臭い', '匂い', '虫', 'カメムシ', '狭い', '広い', '眺望', '海', '鳥羽湾',
  '駅', '送迎', '駐車場', 'バイキング', '子連れ', '赤ちゃん', '家族'
];
...
function tag({ platform, sourceUrl, id, title, body, roomType, planName, reviewDate, stayDate, score, language = 'ja' }) {
  ...
  if (/客室.*露天|部屋.*露天|部屋付き露天|お部屋.*露天|露天風呂付き|露天風呂付/.test(combined)) {
  ...
  if (/大浴場|嬉春亭大浴場|南館大浴場|浴場/.test(bodyText)) {
    bathAreas.add('public_bath');
    signals.add('public_bath_hot_spring');
  }
  if (/露天風呂|野天風呂|風流野天風呂|湯亭|温泉村/.test(bodyText)) {
    bathAreas.add('open_air_public_bath');
    signals.add('public_bath_hot_spring');
  }
  if (/貸切風呂|無料貸切風呂|貸切|しゃこ貝風呂|浮世風呂|瓶風呂|釜風呂|たぬき風呂/.test(bodyText)) {
    bathAreas.add('private_bath');
    signals.add('private_bath_experience');
  }
  if (/家族風呂|風水家族風呂|風水貝殻家族風呂/.test(bodyText)) {
    bathAreas.add('family_bath');
    signals.add('private_bath_experience');
  }
  if (/温泉|榊原温泉|七栗の湯|源泉|泉質|湯質|アルカリ性単純泉|肌触り|つるつる|ツルツル|すべすべ|温ま/.test(bodyText)) {
    signals.add('water_texture');
  }
  if (/温泉感|普通のお湯|薄い|水道水|沸かし湯|温泉ではない|ツルツルしない/.test(bodyText)) signals.add('weak_onsen_feeling');
  if (/塩素|カルキ/.test(bodyText)) signals.add('chlorine_smell');
  if (/混雑|混んで|空いて|静か|待ち|貸切状態|人も少なく|人が少な/.test(bodyText)) signals.add('crowding');
  if (/予約|不要|鍵|説明|チェックイン|案内|送迎|駅|駐車場|館内|迷|分かりにく/.test(bodyText)) signals.add('booking_confusion');

  for (const [name, pattern] of [
    ['temperature_control', /ぬるい|熱い|温度|湯加減/],
    ['cleanliness_aging', /掃除|古い|老朽|臭い|匂い|汚|清潔|くたびれ/],
    ['insects', /虫|蚊|カメムシ/],
    ['view_privacy', /眺望|景色|海|鳥羽湾|港|見え/],
    ['access_booking', /予約|送迎|駅|駐車場|チェックイン|館内|電話|鍵|説明|案内/],
    ['public_bath_size_expectation', /大浴場.*小さ|大浴場.*狭|狭い|広い|温泉村.*期待/],
    ['children_family', /子連れ|赤ちゃん|幼児|子供|孫|家族/],
    ['price_expectation', /高い|価格|値段|コスパ/],
    ['buffet_crowding', /バイキング|解体ショー|混雑|時間をずら/]
  ...
  return {
    ...
    onsen_related_body: bodyKeywords.some((kw) => /温泉|風呂|湯|源泉|泉質|露天|大浴場|貸切|家族風呂|足湯|岩盤浴/.test(kw)),
  ...
  for (let offset = 0; offset < maxReviews; offset += 20) {
    ...
    for (const item of data.contents || []) {
      ...
      reviews.push(tag({
        ...
        sourceUrl: 'https://travel.rakuten.co.jp/HOTEL/4761/review.html',
        id: item.id,
        title: item.title,
        body: item.comment,
...
function parseJalanBlock(block, sourceUrl) {
  ...
  let body = text
    .replace(/伊勢志摩 鳥羽温泉郷 戸田家からの返信[\s\S]*$/, '')
    .replace(/返信日：[\s\S]*$/, '');
  ...
  return tag({
    ...
    roomType: text.match(/(露天風呂付|温泉露天風呂|和室|和洋室|洋室|その他|南館|嬉春亭|客室露天)/)?.[1] || '',
    planName: text.match(/プラン\s*(.*?)\s*(価格帯|和室|和洋室|洋室|その他|投稿日|部屋)/)?.[1] || ''
  });
}

async function collectJalan(maxPages = 7) {
  const reviews = [];
  const stats = {};
  const seen = new Set();
  let visibleTotal = null;
...
async function collectJtb(maxPages = 5) {
  const reviews = [];
  const stats = {};
  const seen = new Set();
  let visibleTotal = null;
  for (let page = 1; page <= maxPages; page++) {
    const url = `https://www.jtb.co.jp/kokunai-hotel/htl/${JTB_ID}/review/${page === 1 ? '' : `?page=${page}`}`;
    const html = await fetchHtml(url);
    const text = norm(new JSDOM(html).window.document.body.textContent);
    visibleTotal ||= Number(text.match(/口コミ\s*\\(\\s*([0-9,]+)\\s*\\)/)?.[1]?.replace(/,/g, '')) || null;
    ...
    stats[`jtb_page_${page}`] = reviews.length - before;
  }
  return { reviews, stats, visibleTotal };
}
...
const jalan = await collectJalan(7);
const jtb = await collectJtb(5);
...
await fs.writeFile(path.join(outDir, `todaya_static_review_tags_${TODAY}.json`), JSON.stringify(rows, null, 2));
await fs.writeFile(path.join(outDir, `todaya_static_review_tags_summary_${TODAY}.json`), JSON.stringify(summary, null, 2));
