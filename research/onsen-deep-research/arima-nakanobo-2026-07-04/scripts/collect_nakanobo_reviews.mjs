...
const terms = [
  ...
  '部屋風呂', '客室風呂', '内風呂', '大浴場', '瑞泉', '爽泉', '貸切', '貸切露天',
  '家族風呂', '寿泉', '源泉', 'かけ流し', '循環', '加水', '加温', '塩素', 'カルキ',
  '温泉感', '濃い', '茶色', '泥', '鉄', 'しょっぱい', 'ぬるい', '熱い', '温度',
  '湯加減', 'サウナ', 'ミストサウナ', '混雑', '空いて', '静か', '予約', '送迎',
  '部屋食', '虫', '掃除', '古い', '臭い', '匂い', '入浴のみ'
];
...
function tag({ platform, sourceUrl, id, title, body, roomType, planName, reviewDate, stayDate, score }) {
  ...
  if (/客室.*露天|部屋.*露天|露天風呂付き|金泉露天|銀泉露天/.test(combined)) {
  ...
  if (/大浴場|瑞泉|爽泉/.test(bodyText)) {
    bathAreas.add('public_bath');
    signals.add('public_bath_hot_spring');
  }
  if (/露天風呂|露天/.test(bodyText) && /大浴場|瑞泉|爽泉/.test(bodyText)) {
    bathAreas.add('open_air_public_bath');
    signals.add('public_bath_hot_spring');
  }
  if (/貸切露天|貸切風呂/.test(bodyText)) {
    bathAreas.add('private_bath');
    signals.add('private_bath_experience');
  }
  if (/家族風呂|寿泉/.test(bodyText)) {
    bathAreas.add('family_bath');
    signals.add('private_bath_experience');
  }
  if (/金泉|銀泉|赤湯|ラドン|源泉|泉質|濃い|茶色|鉄|しょっぱ|泥|温泉成分/.test(bodyText)) {
  ...
  if (/予約|送迎|説明|チェックイン|案内|部屋違い/.test(bodyText)) signals.add('booking_confusion');

  for (const [name, pattern] of [
    ['temperature_control', /ぬるい|熱い|温度|湯加減/],
    ['cleanliness_aging', /掃除|古い|臭い|匂い|汚|老朽/],
    ['insects', /虫|蚊/],
    ['quiet_adults_only', /静か|大人|子供|13歳/],
    ['access_booking', /予約|送迎|駅|チェックイン/],
  ...
  return {
    ...
    onsen_related_body: bodyKeywords.some((kw) => /温泉|金泉|銀泉|露天|風呂|湯|源泉|泉質|サウナ|貸切|家族風呂|瑞泉|爽泉|寿泉/.test(kw)),
...
async function collectRakuten(maxPages = 12) {
  ...
  for (let page = 1; page <= maxPages; page++) {
    const url = page === 1
      ? 'https://travel.rakuten.co.jp/HOTEL/25288/review.html'
      : `https://review.travel.rakuten.co.jp/hotel/voice/25288?page=${page}`;
    ...
    for (const item of contents) {
      ...
      reviews.push(tag({
        ...
        sourceUrl: 'https://travel.rakuten.co.jp/HOTEL/25288/review.html',
        id: item.id,
        title: item.title,
        body: item.comment,
...
function parseJalanBlock(block, sourceUrl) {
  ...
  let body = text
    .replace(/有馬温泉　中の坊瑞苑からの返信[\s\S]*$/, '')
    .replace(/中の坊瑞苑からの返信[\s\S]*$/, '')
    .replace(/返信日：[\s\S]*$/, '');
  ...
  return tag({
    ...
    roomType: text.match(/(和洋室|和室|ツイン|露天風呂付き客室|貴賓室|洋室)/)?.[1] || '',
    planName: text.match(/プラン\s*(.*?)\s*(和洋室|和室|ツイン|朝・夕|朝のみ|夕のみ|食事なし)/)?.[1] || ''
  });
}

async function collectJalan(maxArchivePages = 5) {
  const reviews = [];
  const stats = {};
  const seen = new Set();
  const urls = ['https://www.jalan.net/yad342576/kuchikomi/'];
  urls.push('https://www.jalan.net/yad342576/kuchikomi/archive/');
  for (let page = 2; page <= maxArchivePages; page++) urls.push(`https://www.jalan.net/yad342576/kuchikomi/archive/${page}.HTML`);
...
const jalan = await collectJalan(5);
const reviews = [...rakuten.reviews, ...jalan.reviews];
const stats = { ...rakuten.stats, ...jalan.stats };
const summary = summarize(reviews, stats);
await fs.writeFile(path.join(outDir, 'nakanobo_static_review_tags_2026-07-04.json'), JSON.stringify({ summary, reviews }, null, 2));
await fs.writeFile(path.join(outDir, 'nakanobo_static_review_tags_summary_2026-07-04.json'), JSON.stringify(summary, null, 2));
