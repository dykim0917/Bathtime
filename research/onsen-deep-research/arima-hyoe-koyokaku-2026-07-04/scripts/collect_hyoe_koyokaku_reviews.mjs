...
const terms = [
  '温泉', '金泉', '銀泉', '赤湯', '露天', '露天風呂', '客室露天', '部屋露天',
  '露天風呂付', '部屋風呂', '客室風呂', '内風呂', '大浴場', '一の湯', '二の湯',
  '三の湯', '湯巡り', '貸切', '貸切露天', '貸切風呂', '家族風呂', '朝霧', '夕霧',
  '源泉', 'かけ流し', '循環', '加水', '加温', '塩素', 'カルキ', '温泉感', '濃い',
  '茶色', '鉄', 'しょっぱい', 'ぬるい', '熱い', '温度', '湯加減', 'サウナ',
  '混雑', '混んで', '空いて', '静か', '眺望', '景色', '夜景', '予約', '送迎',
  'シャトル', '坂', '館内', '遠い', '階段', '日帰り', '入浴', '虫', '掃除',
  '古い', '臭い', '匂い', '浮いて'
];
...
function tag({ platform, sourceUrl, id, title, body, roomType, planName, reviewDate, stayDate, score }) {
  ...
  if (/客室.*露天|部屋.*露天|露天風呂付き|露天風呂付|露天.*客室/.test(combined)) {
  ...
  if (/大浴場|一の湯|二の湯|三の湯|湯巡り/.test(bodyText)) {
    bathAreas.add('public_bath');
    signals.add('public_bath_hot_spring');
  }
  if (/露天風呂|露天/.test(bodyText) && /大浴場|一の湯|二の湯|三の湯|湯巡り/.test(bodyText)) {
    bathAreas.add('open_air_public_bath');
    signals.add('public_bath_hot_spring');
  }
  if (/貸切露天|貸切風呂|貸切|朝霧|夕霧/.test(bodyText)) {
  ...
  if (/金泉|銀泉|赤湯|源泉|泉質|濃い|茶色|鉄|しょっぱ|温泉成分/.test(bodyText)) {
  ...
  if (/予約|送迎|シャトル|坂|説明|チェックイン|案内|部屋違い|日帰り|館内|遠い|階段/.test(bodyText)) signals.add('booking_confusion');

  for (const [name, pattern] of [
    ['temperature_control', /ぬるい|熱い|温度|湯加減/],
    ['cleanliness_aging', /掃除|古い|臭い|匂い|汚|老朽|浮いて/],
    ['insects', /虫|蚊/],
    ['view', /眺望|景色|夜景|見晴らし|山々|温泉街/],
    ['access_booking', /予約|送迎|シャトル|駅|坂|チェックイン|館内|遠い|階段/]
  ...
  return {
    ...
    onsen_related_body: bodyKeywords.some((kw) => /温泉|金泉|銀泉|露天|風呂|湯|源泉|泉質|サウナ|貸切|家族風呂|一の湯|二の湯|三の湯|朝霧|夕霧|入浴/.test(kw)),
...
async function collectRakuten(maxPages = 12) {
  const reviews = [];
  const stats = {};
  const seen = new Set();
  for (let page = 1; page <= maxPages; page++) {
    const url = page === 1
      ? 'https://travel.rakuten.co.jp/HOTEL/8636/review.html'
      : `https://review.travel.rakuten.co.jp/hotel/voice/8636?page=${page}`;
    const html = await fetchHtml(url);
    const contents = parseRakutenState(html)?.reviewList?.data?.contents || [];
    ...
    for (const item of contents) {
      ...
      reviews.push(tag({
        ...
        sourceUrl: 'https://travel.rakuten.co.jp/HOTEL/8636/review.html',
        id: item.id,
        title: item.title,
        body: item.comment,
...
function parseJalanBlock(block, sourceUrl) {
  ...
  return tag({
    ...
    roomType: text.match(/(和洋室|和室|ツイン|露天風呂付き客室|露天風呂付和モダン客室|北館|西館|南館)/)?.[1] || '',
...
async function collectJalan() {
  ...
  const stats = {};
  const seen = new Set();
  const urls = [0, 30, 60, 90, 120].map((idx) => idx === 0
    ? 'https://www.jalan.net/yad321542/kuchikomi/'
    : `https://www.jalan.net/yad321542/kuchikomi/?screenId=UWW3701&idx=${idx}`);
...
async function collectJtb(maxPages = 8) {
  ...
  const seen = new Set();
  for (let page = 1; page <= maxPages; page++) {
    const url = 'https://www.jtb.co.jp/kokunai-hotel/htl/6435001/review/?area=28&room=1&dateunspecified=1&staynight=1&roomassign=m2'
      + (page > 1 ? `&page=${page}` : '');
    const html = await fetchHtml(url);
...
const rakuten = await collectRakuten(12);
const jalan = await collectJalan();
const jtb = await collectJtb(8);
...
await fs.writeFile(path.join(outDir, `hyoe_koyokaku_static_review_tags_${TODAY}.json`), JSON.stringify({ summary, reviews }, null, 2));
await fs.writeFile(path.join(outDir, `hyoe_koyokaku_static_review_tags_summary_${TODAY}.json`), JSON.stringify(summary, null, 2));
