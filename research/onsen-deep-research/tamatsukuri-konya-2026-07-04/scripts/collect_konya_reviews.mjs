...
const RAKUTEN_ID = 106267;
const JALAN_ID = 331176;
const JTB_ID = '7323010';

const terms = [
  '温泉', '玉造温泉', 'ホテル玉泉', '玉泉', '源泉', '泉質', '湯質',
  '硫酸塩泉', '塩化物泉', '単純温泉',
  '美肌', '美肌の湯', 'つるつる', 'ツルツル', 'すべすべ', '肌触り',
  '大浴場', '神戸湯殿', '檜の湯', '巌の湯', '曲水の庭',
  '露天', '露天風呂', '露天風呂エリア', '展望風呂',
...
function tag({ platform, sourceUrl, id, title, body, roomType, planName, reviewDate, stayDate, score, language = 'ja' }) {
  ...
  if (/大浴場|神戸湯殿|檜の湯|巌の湯|浴場|お風呂|風呂/.test(bodyText)) {
  ...
  for (let offset = 0; offset < maxReviews; offset += 20) {
    ...
    for (const item of data.contents || []) {
      ...
      reviews.push(tag({
        ...
        sourceUrl: 'https://travel.rakuten.co.jp/HOTEL/106267/review.html',
        id: item.id,
        title: item.title,
        body: item.comment,
...
await fs.writeFile(path.join(outDir, `konya_static_review_tags_${TODAY}.json`), JSON.stringify(rows, null, 2));
await fs.writeFile(path.join(outDir, `konya_static_review_tags_summary_${TODAY}.json`), JSON.stringify(summary, null, 2));
