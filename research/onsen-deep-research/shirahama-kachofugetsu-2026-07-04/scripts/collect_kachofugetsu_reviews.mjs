...
const RAKUTEN_ID = 183409;
const JALAN_ID = 351872;

const terms = [
  '温泉', '白浜温泉', '花鳥風月', '源泉', '泉質', '湯質',
  'ナトリウム塩化物泉', '塩化物泉', '天然温泉',
  '美肌', '美肌の湯', 'つるつる', 'ツルツル', 'すべすべ', '肌触り',
  '大浴場', '大浴場なし', '大浴場がない', '大浴場はない',
  '露天', '露天風呂', '半露天', '半露天風呂', '展望風呂',
  '客室露天', '客室半露天', '露天風呂付き', '露天風呂付', '半露天風呂付', '部屋風呂', '客室風呂', '内湯', '陶器風呂',
  '貸切', '貸切風呂', '家族風呂', 'サウナ', 'ぬるい', '熱い', '温度', '湯加減',
  '塩素', 'カルキ', '温泉感', '混雑', '混んで', '空いて', '待ち',
  '予約', '説明', '案内', '清潔', '掃除', '古い', '老朽', '臭い', '匂い',
  '虫', '蚊', 'カメムシ', '狭い', '広い', '眺望', '海', '白良浜', '駅', '送迎', '駐車場',
  'バイキング', '子連れ', '赤ちゃん', '家族', 'ドリンク', '冷蔵庫'
];
...
function tag({ platform, sourceUrl, id, title, body, roomType, planName, reviewDate, stayDate, score, language = 'ja' }) {
  ...
  if (/客室.*露天|客室.*半露天|部屋.*露天|部屋.*半露天|部屋付き露天|お部屋.*露天|お部屋.*半露天|露天風呂付き|露天風呂付|半露天風呂付|半露天/.test(combined)) {
    bathAreas.add('room_open_air_bath');
    signals.add('room_bath_hot_spring');
  }
  if (/部屋風呂|客室風呂|部屋の風呂|内風呂|内湯|陶器風呂|部屋.*風呂/.test(combined)) {
    bathAreas.add('room_bath');
    signals.add('room_bath_hot_spring');
  }
  const publicBathAbsence = /大浴場\s*(なし|がない|はない|無し)|大浴場.*(ありません|ない|無い|設けず)|大浴場ではなく|大浴場じゃなく/.test(bodyText);
  if (/大浴場|共同浴場|共用風呂/.test(bodyText) && !publicBathAbsence) {
    bathAreas.add('public_bath');
    signals.add('public_bath_hot_spring');
  }
  if (/露天風呂|半露天風呂/.test(bodyText) && /大浴場|共同浴場|共用風呂/.test(bodyText) && !publicBathAbsence) {
    bathAreas.add('open_air_public_bath');
    signals.add('public_bath_hot_spring');
  }
  if (/貸切風呂|貸切露天/.test(bodyText) && !/欲しい|あれば|ない|無し|なし/.test(bodyText)) {
  ...
  if (/温泉|白浜温泉|天然温泉|源泉|泉質|湯質|美肌|美肌の湯|塩化物泉|肌触り|つるつる|ツルツル|すべすべ|温ま/.test(bodyText)) {
  ...
  for (const [name, pattern] of [
    ...
    ['insects', /虫|蚊|カメムシ/],
    ['view_privacy', /眺望|景色|海|白良浜|見え|展望|目隠し|プライベート|視線/],
    ['access_booking', /予約|送迎|駅|駐車場|チェックイン|館内|電話|鍵|説明|案内/],
    ['public_bath_absence', /大浴場なし|大浴場がない|大浴場はない|大浴場.*無|大浴場.*設けず|大浴場ではなく/],
  ...
  for (let offset = 0; offset < maxReviews; offset += 20) {
    ...
    for (const item of data.contents || []) {
      ...
      reviews.push(tag({
        ...
        sourceUrl: 'https://travel.rakuten.co.jp/HOTEL/183409/review.html',
        id: item.id,
        title: item.title,
        body: item.comment,
...
function parseJalanBlock(block, sourceUrl) {
  ...
  let body = text
    .replace(/南紀白浜 和みの湯 花鳥風月.*?からの返信[\s\S]*$/, '')
    .replace(/花鳥風月.*?からの返信[\s\S]*$/, '')
    .replace(/返信日：[\s\S]*$/, '');
  ...
  return tag({
    ...
    roomType: text.match(/(半露天風呂付|露天風呂付|温泉露天風呂|スタンダード洋室|ツイン|和洋室|洋室|客室露天)/)?.[1] || '',
    planName: text.match(/プラン\s*(.*?)\s*(価格帯|ツイン|和洋室|洋室|その他|投稿日|部屋)/)?.[1] || ''
  });
}
...
await fs.writeFile(path.join(outDir, `kachofugetsu_static_review_tags_${TODAY}.json`), JSON.stringify(rows, null, 2));
await fs.writeFile(path.join(outDir, `kachofugetsu_static_review_tags_summary_${TODAY}.json`), JSON.stringify(summary, null, 2));
