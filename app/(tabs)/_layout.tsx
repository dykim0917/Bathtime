...
function TabBarIcon({ name, color, focused }: { name: TabIconName; color: string; focused: boolean }) {
  ...
  const activeFill = focused ? color : 'none';
  const cutoutColor = archiveColors.surface;

  return (
    <Svg width={22} height={22} viewBox="0 0 24 24" fill="none" style={{ marginBottom: -2 }}>
      {name === 'home' ? (
        <Path d="M4 10.7 12 4l8 6.7V20a1 1 0 0 1-1 1h-5v-6h-4v6H5a1 1 0 0 1-1-1v-9.3Z" fill={activeFill} stroke={color} strokeWidth={strokeWidth} strokeLinejoin="round" />
      ) : null}
      {name === 'explore' ? (
        <>
          <Circle cx="12" cy="12" r="8.2" stroke={color} strokeWidth={strokeWidth} />
          <Path d="m15.4 8.6-2 5-5 2 2-5 5-2Z" fill={activeFill} stroke={color} strokeWidth={strokeWidth} strokeLinejoin="round" />
        </>
      ) : null}
      {name === 'routine' ? (
        <>
          ...
          <Circle cx="12" cy="12" r="3.3" fill={activeFill} stroke={color} strokeWidth={strokeWidth} />
        </>
      ) : null}
      {name === 'submit' ? (
        <>
          <Path d="M5 6.5A2.5 2.5 0 0 1 7.5 4h9A2.5 2.5 0 0 1 19 6.5v6.7a2.5 2.5 0 0 1-2.5 2.5H11l-4.5 3.1v-3.1A2.5 2.5 0 0 1 5 13.2V6.5Z" fill={activeFill} stroke={color} strokeWidth={strokeWidth} strokeLinejoin="round" />
          <Path d="M12 7.8v4.8M9.6 10.2h4.8" stroke={focused ? cutoutColor : color} strokeWidth={strokeWidth} strokeLinecap="round" />
        </>
      ) : null}
      {name === 'saved' ? (
        <Path d="M7 5.2A2.2 2.2 0 0 1 9.2 3h5.6A2.2 2.2 0 0 1 17 5.2V21l-5-3-5 3V5.2Z" fill={activeFill} stroke={color} strokeWidth={strokeWidth} strokeLinejoin="round" />
      ) : null}
    </Svg>
  );
}
