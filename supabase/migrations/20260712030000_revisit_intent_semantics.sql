...
AS $$
  ...
  SELECT
    ...
    count(*) FILTER (WHERE filtered.revisit_intent = 'yes'),
    count(*) FILTER (WHERE filtered.revisit_intent IN ('yes', 'no')),
...
COMMENT ON FUNCTION public.read_public_onsen_review_aggregate(TEXT, TEXT) IS
  'Aggregate first-party public review signals; only explicit yes/no answers count toward revisit intent.';
