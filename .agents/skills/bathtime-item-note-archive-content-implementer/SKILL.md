...
If the existing upsert script supports `item-seed.*`, use it directly. If it only supports `spot-seed.*`, do not rename silently. Report the mismatch and either adapt with a generated item-specific artifact or leave a code note.
